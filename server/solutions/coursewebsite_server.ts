...
import { userData } from './structure';
import { credentialInput,courseStruct,credInput,coursestructure } from '@abhishek356/common';
...
let ADMINS:credentialInput[] = [];
let USERS:userData[] = [];
let COURSES:courseStruct[] = [];
let currentlyLoggedInUser:string;
...
// Admin routes
app.post('/admin/signup', (req:Request, res:Response) => {

  const parsedInput = credInput.safeParse(req.body)
...
app.post('/admin/me',authenticateJwt,(req:Request,res:Response)=>{
...
app.post('/admin/courses', authenticateJwt, (req:Request, res:Response) => {
  let parsedCourseInput = coursestructure.safeParse(req.body) 

  if(!parsedCourseInput.success)
  {
   return res.status(411).json({msg: 'wrong input from the courses'})
  }
  const course = parsedCourseInput.data;
...
app.put('/admin/courses/:courseId', authenticateJwt, (req:Request, res:Response) => {
...
app.get('/admin/courses', authenticateJwt, (req:Request, res:Response) => {
  res.json({ courses: COURSES });
});

app.get('/admin/getCourse/:courseId',authenticateJwt, (req:Request,res:Response)=>{
...
app.post('/users/signup', (req:Request, res:Response) => {
  ...
  let parsedInput = credInput.safeParse(req.body)
  ...
  if (user) {
    ...
    const newUser:userData = { username, password, purchasedCourses};
    USERS.push(newUser);
...
app.get('/users/courses', authenticateJwt, (req:Request, res:Response) => {
  console.log(`request received from the frontend to get all the courses, and
  the list of courses is `,COURSES)
  res.json( COURSES );
});

app.post('/users/courses/:courseId', authenticateJwt, (req:Request, res:Response) => {
  const course = COURSES.find(c => c.id === parseInt(req.params.courseId));
  if (course) {
...
app.get('/users/purchasedCourses', authenticateJwt, (req:Request, res:Response) => {
  const user = USERS.find(u => u.username === req.headers.user);
//  const user = USERS.find(u => u.username === req.user.username);
...
app.post('/user/me',authenticateJwt,(req:Request,res:Response)=>{
...
app.post('/loadCourses',(req:Request,res:Response)=>{
